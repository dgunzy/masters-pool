    ...
    if (year === "2024") {
      ...
      Object.keys(entry).forEach((group) => {
        if (group === "1RL Payout") {
          ...
          // Detect Mutt / Old Mutt groups by their number
...
function matchGolferAndAssignPayout(golferName, group, entry, payouts) {
  ...
  if (golferName.includes("højgaard")) {
    ...
    payoutName = Object.keys(payouts).find((key) => {
      ...
      if (!k.includes("højgaard")) return false;
      return (
        (golferName.includes("nicolai") && k.includes("nicolai")) ||
        (golferName.includes("rasmus")  && k.includes("rasmus"))
      );
    });
    ...
    // Default to Tom Kim; fall back to Michael Kim if Tom isn't in the field
